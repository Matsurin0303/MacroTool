...
| 1-1-1 | Confirmation | 未保存マクロ確認画面 | MessageBox | 未保存のマクロがあるときにクリックすると出現 |  |
| 1-1-1-1 | Yes | はい | Button | マクロの保存を行い、新規マクロを作成する |  |
| 1-1-1-2 | No | いいえ | Button | マクロを保存せず、新規に作成する |  |
| 1-1-1-3 | Cancel | キャンセル | Button | キャンセル |  |
| 1-2 | Open | 既存マクロを開く | Button | 既存のマクロファイルを開く |  |
| 1-2-1 | Confirmation | 未保存マクロ確認画面 | MessageBox | 未保存のマクロがあるときにクリックすると出現 |  |
| 1-2-1-1 | Yes | はい | Button | マクロの保存を行い、既存マクロを開く |  |
| 1-2-1-2 | No | いいえ | Button | マクロを保存せず、既存マクロを開く |  |
| 1-2-1-3 | Cancel | キャンセル | Button | キャンセル |  |
| 1-3 | Recent Files | 最近開いたマクロを表示 | Button | カーソルを合わせると、最近開いたマクロが表示される。件数は10件まで。Open / Recent Files で正常に読込完了した場合、および Save / Save As 成功時に先頭へ更新する。 | 重複パスは既存項目を削除して先頭へ再追加する。存在しないファイルを選択した場合はエラー表示し、その項目をRecent Filesから削除する。 |
| 1-4 | Save | マクロを保存 | Button | マクロを保存、新規の場合は名前を付けて保存 |  |
| 1-4-1 | Confirmation | 未保存マクロ確認画面 | MessageBox | 未保存のマクロがあるときにクリックすると出現 |  |
| 1-4-1-1 | Yes | はい | Button | マクロの保存を行い、既存マクロを開く |  |
| 1-4-1-2 | No | いいえ | Button | マクロを保存せず、既存マクロを開く |  |
...
| 2-2-1-1 | Confirmation | 未保存マクロ確認画面 | MessageBox | 未保存のマクロがあるときにクリックすると出現 |  |
| 2-2-1-1-1 | Yes | はい | Button | マクロの保存を行い、新規マクロを開く |  |
| 2-2-1-1-2 | No | いいえ | Button | マクロを保存せず、新規マクロを開く |  |
...
| 3-2-1-1 | Confirmation | 未保存マクロ確認画面 | MessageBox |  |  |
| 3-2-1-1-1 | Yes | はい | Button |  |  |
| 3-2-1-1-2 | No | いいえ | Button |  |  |

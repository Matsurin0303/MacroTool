...
- `PlaybackAppService`：再生系（Play/Stop/PlayUntilSelected 等、現行対象のみ）
- `SettingsAppService`：設定系（設定読込 / 保存 / リセット、現行対象のみ）
...
### 設定
- UC-31 Open Settings（設定読込）
- UC-32 Save Settings（設定保存）
- UC-33 Reset Settings（設定画面の全項目をデフォルト値へ戻す）
- UC-34 Cancel Settings（未保存変更の破棄）
...
## 入力
- `csvPath`
- `targetMacroId`（現在編集中のMacroを示す）
- `insertAtSelectedRow`（選択行位置から取り込む）
...
## 事後条件
- 現在編集中のMacroDocumentへCSV内容が選択行位置から追加される
...
3. 行→Step(Action)へ変換
4. **Label重複補正**（以下の規則に従う）
   - 4.1 重複検出：インポート対象のStepに含まれるLabel と、現在編集中のMacro内の既存Labelを比較
   - 4.2 重複時の補正ルール：
     - Domain_Model.md「6.2 Label一意性」に準ずる
     - 末尾に連番を付与して一意化（例：`Jump先` → `Jump先1` → `Jump先2`）
     - 末尾が数字の場合はインクリメント（例：`Step1` → `Step2`）
   - 4.3 補正対象：
     - インポート側Labelが空の場合：そのまま保持（Labelなしとして扱う）
     - インポート側Labelが既存と重複：末尾に連番付与して一意化
5. 現在編集中のMacroへ選択行位置からStepを挿入
6. **補正結果を含む** MacroDocumentDto を返却
   - UI では補正されたLabel一覧を表示可能にする
   - ユーザーが補正内容を確認・編集可能にする（Optional）

## エラー
- 列不足/型不正 → ApplicationError
- 補正後も不変条件を満たさない → DomainError（例：無限ループ参照など）

## 補足
- Label補正は**自動で実施される**（エラー扱いではない）
- ユーザーへの通知方法は UI_Spec.md に委譲

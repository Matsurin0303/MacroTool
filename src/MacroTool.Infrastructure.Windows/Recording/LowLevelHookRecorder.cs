...
public sealed class LowLevelHookRecorder : IRecorder, IDisposable
{
    ...
    private IntPtr MouseHookCallback(int nCode, IntPtr wParam, IntPtr lParam)
    {
        if (nCode >= 0 && IsRecording)
        {
            ...
            // v1.0: クリック/ホイールのみを記録（MouseMoveは大量になるため記録しない）
            MacroAction? action = msg switch
            {
                Win32.WM_LBUTTONDOWN => new MouseClickAction
                {
                ...
                Win32.WM_RBUTTONDOWN => new MouseClickAction
                {
                ...
                Win32.WM_MBUTTONDOWN => new MouseClickAction
                {
                ...
                Win32.WM_MOUSEWHEEL => new MouseWheelAction
                {
                    Orientation = WheelOrientation.Vertical,
                    Value = unchecked((short)((info.mouseData >> 16) & 0xFFFF))
                },

                Win32.WM_MOUSEHWHEEL => new MouseWheelAction
                {
    ...
    private IntPtr KeyboardHookCallback(int nCode, IntPtr wParam, IntPtr lParam)
    {
        if (nCode >= 0 && IsRecording)
        {
            ...
            if (msg == Win32.WM_KEYDOWN || msg == Win32.WM_KEYUP)
            {
                ...
                {
                    Option = (msg == Win32.WM_KEYDOWN) ? KeyPressOption.Down : KeyPressOption.Up,
                    Key = vk,
                    Count = 1
                };
